        ...
        <div v-else-if="filteredAndSortedGames.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          >
            ...
            <div class="relative aspect-[460/215] overflow-hidden bg-neutral-950 shrink-0">
              ...
              <!-- Hover Play Launcher Action -->
              <a 
                :href="'steam://run/' + game.appid"
                class="absolute inset-0 bg-neutral-950/70 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-2 group/btn cursor-pointer"
              >
                <div class="p-3 rounded-full bg-cyan-500 text-neutral-950 scale-75 group-hover/btn:scale-100 transition-transform duration-300 shadow-lg shadow-cyan-500/40">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 fill-current" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <polygon points="5 3 19 12 5 21 5 3"></polygon>
                  </svg>
                </div>
                <span class="absolute bottom-4 text-xs font-semibold text-cyan-400 tracking-wide uppercase">Launch Game</span>
              </a>
            </div>
...
onMounted(() => {
  ...
  // Fetch games (if keys exist in storage OR if configured in env)
  fetchGames()
})
