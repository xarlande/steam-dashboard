    ...
    <template #header-left>
      ...
      <section class="mb-8 animate-fade-in">
        <div class="flex items-center gap-3">
          <div class="p-2.5 rounded-xl bg-violet-500/10 text-violet-400 border border-violet-500/20 shrink-0">
          ...
          <div>
            ...
            <p class="text-xs sm:text-sm text-muted-foreground font-medium">{{ $t('analytics.description') }}</p>
          </div>
        </div>
      </section>
        ...
        <div class="p-6 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive-foreground text-sm flex items-start gap-4 shadow-lg">
          ...
          <div class="flex-1">
            ...
            <div class="mt-4 flex items-center gap-3">
              ...
              <UiButton variant="outline" @click="fetchGames">
                Retry
              </UiButton>
            </div>
          </div>
        ...
        <UiCard class="overflow-hidden bg-gradient-to-br from-card to-card/50">
          <UiCardContent class="p-6">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-border/60">
              <h2 class="text-sm font-bold text-muted-foreground uppercase tracking-widest">{{ $t('analytics.title') }}</h2>

              <!-- Period Toggle Switch -->
              <div class="flex items-center bg-muted/65 p-1 rounded-xl border border-border/65 shadow-inner">
                <button
                  @click="analyticsPeriod = 'recent'"
                  class="text-xs font-bold px-3 py-1.5 rounded-lg transition-all"
                  :class="analyticsPeriod === 'recent' ? 'bg-card text-foreground shadow-xs border border-border/30' : 'text-muted-foreground hover:text-foreground'"
                >
                ...
                <button
                  ...
                  class="text-xs font-bold px-3 py-1.5 rounded-lg transition-all"
                  :class="analyticsPeriod === 'allTime' ? 'bg-card text-foreground shadow-xs border border-border/30' : 'text-muted-foreground hover:text-foreground'"
                >
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-8 pt-6">
              ...
              <div class="md:col-span-2 flex flex-col items-center justify-center p-5 rounded-2xl bg-card/25 border border-border/60 relative">
                <h3 class="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-6 self-start">{{ $t('analytics.balanceTitle') }}</h3>
                ...
                <div class="relative w-44 h-44 flex items-center justify-center select-none">
                  <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                    ...
                    <circle cx="50" cy="50" r="40" stroke="currentColor" stroke-width="10" fill="transparent" class="text-neutral-100 dark:text-neutral-850" />
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
                    <span class="text-2xl font-black tracking-tight text-foreground">{{ activeStoryPercent }}%</span>
                    <span class="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mt-0.5">🎭 Story</span>
                  </div>
                </div>

                <!-- Legend details -->
                <div class="w-full mt-6 space-y-2.5 pt-4 border-t border-border/60">
                  <div v-if="activeTotalHours === 0" class="text-center text-xs text-muted-foreground py-2 font-medium">
                  ...
                  <template v-else>
                    ...
                    <div class="flex items-center justify-between text-xs">
                      ...
                      <span class="font-extrabold text-cyan-400">
                        {{ $t('analytics.legendHours', { hours: formatHours(activeStoryHours), percent: activeStoryPercent }) }}
                      </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-xs">
                      ...
                      <span class="font-extrabold text-rose-400">
                        {{ $t('analytics.legendHours', { hours: formatHours(activeSessionHours), percent: activeSessionPercent }) }}
                      </span>
                    </div>
                  </template>
                </div>
              ...
              <div class="md:col-span-3 flex flex-col justify-between p-5 rounded-2xl bg-card/25 border border-border/60">
                <h3 class="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-5">{{ $t('analytics.topGamesTitle') }}</h3>
                
                <div v-if="topFiveGames.length === 0" class="flex-1 flex items-center justify-center text-xs text-muted-foreground py-10 font-medium">
                  {{ $t('analytics.legendNoData') }}
                </div>
                <div v-else class="space-y-4.5 flex-1 flex flex-col justify-center">
                  ...
                  >
                    <!-- Rank Indicator -->
                    <span class="text-xs font-black text-muted-foreground/60 w-3.5 select-none text-center">
                      {{ index + 1 }}
                    </span>

                    <!-- Game Image banner -->
                    <div class="relative w-12 h-6 rounded-md overflow-hidden bg-muted border border-border/40 shrink-0 shadow-xs">
                      <img 
                        :src="game.header_img" 
                        :alt="game.name"
                    ...
                    <div class="flex-1 min-w-0 space-y-1">
                      <div class="flex items-center justify-between text-xs gap-2">
                        <h4 class="font-bold text-foreground truncate group-hover/bar:text-violet-400 transition-colors" :title="game.name">
                          {{ game.name }}
                        </h4>
                        <span class="font-black text-foreground shrink-0 pl-2">
                          {{ formatHours(game.display_hours) }} <span class="text-[10px] text-muted-foreground font-semibold">{{ $t('common.hoursSuffix') }}</span>
                        </span>
                      </div>

                      <!-- Progress bar track -->
                      <div class="h-2 rounded-full bg-neutral-200 dark:bg-neutral-800/80 overflow-hidden shadow-inner flex">
                        <div 
...
async function fetchGames() {
  ...
  try {
    ...
    if (response.success && response.games) {
      ...
      error.value = response.error || 'Failed to fetch games library.'
    }
  } catch (err: any) {
    console.error('Error fetching library details for analytics:', err)
...
const allTimeStoryHours = computed(() => {
  const mins = games.value.reduce((sum, g) => getGameCategory(g) === 'story' ? sum + g.playtime_forever : sum, 0)
  return Math.round((mins / 60) * 10) / 10
})

const allTimeSessionHours = computed(() => {
  const mins = games.value.reduce((sum, g) => getGameCategory(g) === 'session' ? sum + g.playtime_forever : sum, 0)
  return Math.round((mins / 60) * 10) / 10
})
...
const topFiveGames = computed(() => {
  ...
  if (analyticsPeriod.value === 'recent') {
    ...
    const sorted = [...playedRecent].sort((a, b) => (b.playtime_2weeks || 0) - (a.playtime_2weeks || 0)).slice(0, 5)
    
    return sorted.map(g => {
      const recentHours = Math.round(((g.playtime_2weeks || 0) / 60) * 10) / 10
      return {
        ...g,
        display_hours: recentHours
      }
    })
  } else {
    const sorted = [...games.value].sort((a, b) => b.playtime_forever - a.playtime_forever).slice(0, 5)
    
    return sorted.map(g => {
      return {
        ...g,
        display_hours: g.playtime_hours
      }
    })
  }
})
